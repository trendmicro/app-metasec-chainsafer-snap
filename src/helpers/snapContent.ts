...
export const transactionInsightLayout: OnTransactionHandler = async ({
  ...
  if (transaction) {
    ...
    if (riskSummaryResult.severity == 'caution') {
      displayPanel = panel([simulationPanel, divider(), riskSummaryPanel, divider(), riskPanel])
    } else {
      displayPanel = panel([riskSummaryPanel, divider(), riskPanel, divider(), simulationPanel])
    }
    ...
    result.contracts.forEach(function (contract, index) {
      contractPanel.push(
        text(`${index + 1}.[${contract.contractName}] Contract address 👉[${
          ...
          parseFloat(contract.fee) > 0 ? `▶ ${convertWeiToEth(contract.fee).toString()} ETH` : ''
        } ${Number.isNaN(parseInt(contract.feeDollarValue)) ? '' : `($${contract.feeDollarValue})`}
        `)
      )
    })
  }
